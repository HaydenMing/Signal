...
Replaces CLIP ViT-B-16 visual encoder with DINOv3 ViT-B/16.
...
def load_dinov3_vitb16(pretrained=True):
    """Load a DINOv3 ViT-B/16 from official hub."""
...
def _remap_hf_to_fb(hf_state_dict):
    ...
    for i in range(n_layers):
        ...
        # Q, K, V biases → concatenated (HF has no k_proj.bias, pad with zeros)
        ...
        # Output projection + layer scale + norms + MLP
        for hf_k, fb_k in [
...
class DINOv3Encoder(nn.Module):
    """
    Wraps DINOv3 ViT to match the interface expected by build_transformer.

    Input:  image tensor [B, 3, H, W]
    Output: (patch_tokens [B, N, 768], cls_token [B, 768])  — same order as CLIP's return

    Usage:
        encoder = DINOv3Encoder(pretrained_path='/path/to/weights')
    ...
    def __init__(self, pretrained_path=None):
        ...
        if pretrained_path is None or pretrained_path == '':
            ...
            if os.path.exists(sf_path):
                ...
                print(f'Loaded DINOv3 from safetensors (HF→FB): {pretrained_path}')
                if missing:
                    print(f'  Missing keys: {missing}')
                if unexpected:
                    print(f'  Unexpected keys: {unexpected}')
            else:
                raise FileNotFoundError(f'No model.safetensors in: {pretrained_path}')
        elif os.path.exists(pretrained_path):
            self.backbone = load_dinov3_vitb16(pretrained=False)
            state_dict = torch.load(pretrained_path, map_location='cpu')
            self.backbone.load_state_dict(state_dict, strict=True)
            print(f'Loaded DINOv3 from .pth: {pretrained_path}')
        else:
    ...
    def forward(self, x, cv_embed=None):
        """
        Args:
            x:        [B, 3, H, W]
            cv_embed: [B, 768] or [B, 1, 768] — camera SIE embedding
        Returns:
        ...
        if cv_embed is not None:
            if cv_embed.dim() == 3:
                cv_embed = cv_embed.squeeze(1)       # [B, 1, 768] → [B, 768]
